...
# Termoso copy of tauri-apps/linuxdeploy-plugin-gtk (master, dda522bc),
# the AppImage plugin the Tauri bundler downloads into ~/.cache/tauri.
# `prepare.sh` puts this file there first so the bundler uses it instead.
# Local changes, both marked "TERMOSO":
#   * the bundled libwayland-* are removed again: the copy from the build
#     host shadows the system one and a newer Mesa's libEGL_mesa then fails
#     to load (missing wl_* symbols), so WebKitWebProcess aborts with
#     "Could not create default EGL display: EGL_BAD_PARAMETER" and the
#     window stays blank (tauri-apps/tauri#15665, mesa/mesa#11316);
#   * GDK_BACKEND=x11 is a default rather than forced, so
#     `GDK_BACKEND=wayland ./Termoso.AppImage` runs natively on Wayland.
...
export GDK_BACKEND="${GDK_BACKEND:-x11}" # TERMOSO: default, not forced; upstream forces x11 because of https://github.com/tauri-apps/tauri/issues/8541
...
# TERMOSO: never ship libwayland; the host's Mesa/EGL must see the host's copy.
echo "Removing bundled libwayland"
find "$APPDIR"/usr/lib* \( -type f -o -type l \) -name 'libwayland-*.so*' -print -delete
