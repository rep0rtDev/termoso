...
export function aiErrorText(e: unknown): { text: string; retry: boolean } {
  if (isDesktopError(e)) {
    switch (e.kind) {
      case "ai_not_enabled":
        return { text: "AI suggestions are turned off for this account.", retry: false };
      case "ai_quota_exceeded":
        return { text: "Today's quota is used up. It resets at midnight UTC.", retry: false };
      case "ai_busy":
        return { text: "The AI provider is busy. Try again in a moment.", retry: true };
      case "ai_unavailable":
        return { text: "The AI provider did not answer. Nothing was inserted.", retry: true };
      case "unauthorized":
        return { text: "Sign in again to use AI suggestions.", retry: false };
    }
  }
...
export function providerLabel(s: Pick<AiStatus, "provider" | "model">): string {
  return [s.provider, s.model].filter((x): x is string => !!x).join(" · ") || "AI provider";
}
...
export function contextLabel(pane: { protocol: string | null; shell: string | null }): string {
  const os = pane.protocol === "local" ? "this computer" : "host OS";
